...
mod test {
    ...
    fn get_merkle_tree(input: &Vec<ArkFrBN254>, slice_size: u32) -> HostOrDeviceSlice<'static, IcicleFrBN254> {
        let n = input.len();
        let mut tree_slice = if slice_size <= 0 {
            HostOrDeviceSlice::cuda_malloc(2 * n - 1).unwrap()
        } else {
            let slice_count = n / (slice_size as usize);
            HostOrDeviceSlice::cuda_malloc(n + 2 * slice_count - 1).unwrap()
        };
        ...
        build_merkle_tree(&device_config, &mut tree_slice, n as u32, slice_size).unwrap();
    ...
    /// Returns an array with length `slice_size` that represents the slice in the Merkle tree and
    /// the path to the root.
    ///
    /// # Arguments
    ///
    /// * `index` - the index of the leaf in the Merkle tree. The return slice values from this
    /// method must contain this leaf.
    fn get_merkle_path_with_slice(
        tree_slice: &HostOrDeviceSlice<IcicleFrBN254>,
        n: usize,
        index: usize,
        slice_size: usize,
    ) -> (Vec<ArkFrBN254>, Vec<ArkFrBN254>) {
        assert!(n % slice_size == 0);
        let slice_index = index / slice_size;

        // Get the slice values.
        let mut slice_values = vec![];
        let start = slice_index * slice_size;
        for i in 0..slice_size {
            slice_values.push(get_device_value_at_index(tree_slice, start + i));
        }

        // Get the path.
        let mut ice_result = vec![];
        let mut x_n = n / slice_size;
        let mut x_index = slice_index;
        let mut offset = n;
        loop {
            if x_n == 1 {
                ice_result.push(get_device_value_at_index(tree_slice, offset + x_index));
    ...
    fn test_build_merkle_tree() {
        ...
        let tree_slice = get_merkle_tree(&input, 0);
    ...
    #[test]
    fn test_build_merkle_tree_for_slice() {
        let input = vec![
            ArkFrBN254::from(1),
            ArkFrBN254::from(2),
            ArkFrBN254::from(3),
        ...
        let tree_slice = get_merkle_tree(&input, slice_size as u32);

        let n = input.len();
        let slice_count = n / slice_size;
        let result = icicles_to_arks(&tree_slice, n + 2 * slice_count - 1)[n + slice_count..].to_vec();
        ...
        for index in 0..input.len() {
            let slice_n = n / slice_size;
            let slice_index = index / slice_size;

            let (leave_values, ice_result) = get_merkle_path_with_slice(&tree_slice, n, index, slice_size);
            let start = slice_index * slice_size;
            assert_eq!(input[start..start + slice_size].to_vec(), leave_values);

            let log_slice_n = slice_n.ilog2() as usize;
            assert_eq!(log_slice_n + 1, ice_result.len());

            if slice_index % 2 == 0 {
                assert_eq!(slice_hashes[slice_index + 1], ice_result[0]);
            } else {
                assert_eq!(slice_hashes[slice_index - 1], ice_result[0]);
            }

            let mut len = slice_n / 2;
            let mut x = slice_index / 2;
            let mut offset = 0;

            for i in 0..log_slice_n {
                if i == log_slice_n - 1 {
                    // This must the be root of the Merkle tree
                    assert_eq!(expected[slice_n - 2], ice_result[i + 1]);
                } else {
                    let cur_index = offset + x;
                    if cur_index % 2 == 0 {
                        assert_eq!(expected[cur_index + 1], ice_result[i + 1]);
