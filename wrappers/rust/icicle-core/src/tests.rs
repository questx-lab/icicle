...
pub fn check_montgomery_conversions<C: CurveConfig>() {
    ...
    println!("First point before: {:?}", affine_points.first().unwrap());

    let mut affine_buffer = DeviceSlice::cuda_malloc(size).unwrap();
    affine_buffer.copy_from_host(&affine_points).unwrap();

    C::affine_to_montgomery(affine_buffer.as_slice());

    let mut affine_montgomery_points = vec![Affine::<C>::zero(); size];
    affine_buffer.copy_to_host(&mut affine_montgomery_points).unwrap();
    println!("First point after mont: {:?}", affine_montgomery_points.first().unwrap());

    C::affine_from_montgomery(affine_buffer.as_slice());
    let mut affine_montgomery_points = vec![Affine::<C>::zero(); size];
    affine_buffer.copy_to_host(&mut affine_montgomery_points).unwrap();
    println!("First point after unmont: {:?}", affine_montgomery_points.first().unwrap());
